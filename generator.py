...
class Generator(nn.Module):
    ...
    def sample(self, batch_size, seq_len, x=None):
        ...
        if x is None:
            flag = True
        if flag:
            x = Variable(torch.rand((batch_size, 1)).long())
        if self.use_cuda:
            x = x.cuda()
    ...
    def sample_conditional(self, batch_size, seq_len, hidden,x=None):
        # May need to concat with noise 
        '''Function to sample on condition of noise'''
